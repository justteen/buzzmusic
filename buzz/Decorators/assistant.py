...
from buzz import BOT_ID, MUSIC_BOT_NAME, app, random_assistant
from buzz.Database import get_assistant, save_assistant
...
async def unban_assistant_(_, CallbackQuery):
    ...
    if not a.can_restrict_members:
        return await CallbackQuery.answer(
            "Saya tidak memiliki izin pengguna untuk ban/unban larangan.  Minta admin mana pun untuk membatalkan pencekalan asisten.",
            show_alert=True,
        )
    else:
        try:
        ...
        return await CallbackQuery.edit_message_text(
            "Asisten Unbanned. Coba memutar sekarang."
        )
        ...
        try:
            ...
            if b.status == "kicked":
                return await message.reply_text(
                    f"Assistant Account[{ASS_ID}] di banned.\nUnban dulu untuk memutar musik bot\n\nUsername: @{ASS_USERNAME}",
                    reply_markup=key,
                )
            if b.status == "banned":
                return await message.reply_text(
            ...
            if message.chat.username:
                try:
                    ...
                    await message.reply_text(
                        f"__Assistant Gagal Bergabung__\n\n**Reason**: {e}"
                    )
                    return
            else:
                try:
