from buzz import BOT_USERNAME, LOG_GROUP_ID, app
from buzz.Database import blacklisted_chats, is_gbanned_user, is_on_off
    ...
    async def wrapper(_, message):
        if message.sender_chat:
            return await message.reply_text(
                "Kamu adalah __Anonymous Admin__ di Group ini!\nAtur ulang ke akun utama"
            )
        blacklisted_chats_list = await blacklisted_chats()
        if message.chat.id in blacklisted_chats_list:
            await message.reply_text(
                f"**CHAT Dalam Daftar Hitam**\n\nChat mu di blacklist oleh pengembang.Tanyakan pada __Pengembang__ untuk daftar putih.\nCheck List Pengembang [From Here](https://t.me/{BOT_USERNAME}?start=sudolist)"
            )
            return await app.leave_chat(message.chat.id)
        if await is_on_off(1):
            if int(message.chat.id) != int(LOG_GROUP_ID):
                return await message.reply_text(
                    f"Bot dalam pemeliharaan"
                )
