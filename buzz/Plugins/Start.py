...
from buzz import ASSIDS, BOT_ID, MUSIC_BOT_NAME, OWNER_ID, SUDOERS, app
from buzz import boottime as bot_start_time
from buzz import db, random_assistant
from buzz.Core.PyTgCalls import buzz
from buzz.Database import (add_nonadmin_chat, add_served_chat,
...
from buzz.Decorators.admins import ActualAdminCB
from buzz.Decorators.permission import PermissionCheck
from buzz.Inline import (custommarkup, dashmarkup, setting_markup,
                          setting_markup2, start_pannel, usermarkup, volmarkup)
from buzz.Utilities.assistant import get_assistant_details
from buzz.Utilities.ping import get_readable_time
        ...
        try:
            if member.id == BOT_ID:
                if chat_id in await blacklisted_chats():
                    await message.reply_text(
                        f"Hushh, Group kamu[{message.chat.title}] telah blacklis!\n\nTanyakan pada pengembang untuk whitelist"
                    )
            ...
            if member.id in OWNER_ID:
                return await message.reply_text(
                    f"Pemilik {MUSIC_BOT_NAME}[{member.mention}] Telah Bergabung."
                )
            if member.id in SUDOERS:
                return await message.reply_text(
                    f"A member of {MUSIC_BOT_NAME}'s Sudo User[{member.mention}] Telah Bergabung."
                )
            return
        except:
            return
...
async def useradd(_, message: Message):
    ...
    await asyncio.gather(
        message.delete(),
        message.reply_text(
            f"Thanks sudah menambahkanku {message.chat.title}.\n{MUSIC_BOT_NAME} telah aktif.\n\nUntuk bantuan atau bantuan apa pun, periksa grup dan saluran dukungan kami.",
            reply_markup=InlineKeyboardMarkup(out[1]),
        ),
    )
...
async def okaybhai(_, CallbackQuery):
    ...
    await CallbackQuery.edit_message_text(
        text=f"Thanks sudah menambahkanku {message.chat.title}.\n{MUSIC_BOT_NAME} telah aktif.\n\nUntuk bantuan atau bantuan apa pun, periksa grup dan saluran dukungan kami.",,
        reply_markup=InlineKeyboardMarkup(out[1]),
    )
...
async def AMS(_, CallbackQuery):
    ...
    if not is_non_admin:
        await CallbackQuery.answer(
            "Mode Perintah Sudah Disetel Hanya untuk Admin", show_alert=True
        )
    else:
        await CallbackQuery.answer("Changes Saved")
...
async def start_markup_check(_, CallbackQuery):
    ...
    if command == "LV":
        ...
        volume = 25
        try:
            await buzz.pytgcalls.change_volume_call(c_id, volume)
    ...
    if command == "DIT":
        ...
        await CallbackQuery.answer(
            f"buzz Disk Usage: {diske}%", show_alert=True
        )
